...
export const deleteDocument = async (
  ...
  const targetDocumentsRef = doc(
    collection(documentRef, 'documents'),
    document.id,
  );

  await deleteDoc(targetDocumentsRef);
  if (
    document.data.isCertificate ||
    document.data.isMemory ||
    document.data.isBittext ||
    document.data.isGlossary ||
    document.data.isStyleSheet
  ) {
