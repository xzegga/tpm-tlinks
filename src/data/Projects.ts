...
export const saveProject = async (
  ...
  try {
    ...
    const code =
      project.department === 'all' ? tenant.code : project.department || 'all';
    ...
    const timeLine = isValid(project.timeLine.toDate())
      ? project.timeLine
      : Timestamp.fromDate(addDays(new Date(), 5));
...
export const updateTranslatorId = async (
  projectId: string,
  translatorId: string,
): Promise<void> => {
  try {
