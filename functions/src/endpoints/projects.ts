 ...
 */
export default async function getProjectsData(db: FirebaseFirestore.Firestore, request: CallableRequest<any>) {
    const { status, monthSelected, yearSelected, requestdb, pagination, lastDoc, newQuery, token, tenant } = request.data as any;
    ...
    try {
        ...
        const paging = pagination !== 'All' ? parseInt(pagination) : snapshot.data().count;
